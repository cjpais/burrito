...
import { merge, pipelineLog } from "../../cognition/pipeline";
...
// TODO REMOVE FILEINFO FROM INPUT. UNESSECARY
    ...
    if (parsedMetadata.success) {
      ...
      if (step.validate) {
        ...
        if (!isValid) {
          ...
          // @ts-ignore
          const newMeta = await step.run(metadata, fileInfo);
      ...
      // @ts-ignore
      const newMeta = await step.run(parsedInputMetadata.data, fileInfo);
      // validate new metadata
      const parsedNewMetadata = step.outputType.safeParse(newMeta);
      if (!parsedNewMetadata.success) {
        throw new Error(`Output Metadata for ${step.name} is not valid`);
